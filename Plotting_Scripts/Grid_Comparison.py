...
folder_path = 'E:\BACKUP_NIHAR_1_OCT_2020\\Nihar\\VIV_tests\\Grid_Domain\\Domain\\'

colors = ['xkcd:black','xkcd:blue','xkcd:red','xkcd:green','xkcd:gray']
ls = ['-','-.','--',':','-']
timeaddD = [8.54,0.0,4.46,-2.6,-0.2,0.25]#np.zeros(5,dtype = float)
timeaddO = [3.31,2.6,-0.9,0.0,-0.35,0.0]#np.zeros(5,dtype = float)
...
ap = InsetPosition(ax,[0.35,0.65,0.3,0.25])
bp = InsetPosition(bx,[0.25,0.65,0.3,0.3])
axins.set_axes_locator(ap)
bxins.set_axes_locator(bp)
mark_inset(ax,axins,loc1=3,loc2=4,fc="none",ec='0.5')
mark_inset(bx,bxins,loc1=3,loc2=4,fc="none",ec='0.5')
Ut.plotparams(bxins, '','',[816.8,818.6], [0.84,1.05],True,True,0.06,True,True,0.5)
Ut.plotparams(axins, '','',[822.5,824], [1.38,1.5],True,True,0.025,True,True,0.5)
...
while i < len(subfolders):
    ...
    sti = np.searchsorted(time,800)
    edi = np.searchsorted(time,850)
...
ax.legend([foldernames[0],foldernames[1],foldernames[2],foldernames[3]])
bx.legend([foldernames[0],foldernames[1],foldernames[2],foldernames[3]])

Ut.plotparams(ax,'y','time',[800,850],[-2.0,4.0],False,True,1.0,True,True,5.0)
Ut.plotparams(bx,'y','time',[800,850],[-1.25,2.75],True,True,1.0,True,True,5.0)
...
fig.savefig('E:\\BACKUP_NIHAR_1_OCT_2020\\POF1\Grid.pdf', dpi=150,format='pdf',bbox_inches='tight', pad_inches=0.1)
fig.savefig('E:\\BACKUP_NIHAR_1_OCT_2020\\POF1\Grid.png', dpi=150,format='png',bbox_inches='tight', pad_inches=0.1)
